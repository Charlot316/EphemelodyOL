                ...
                <div class="div3">
                    <div class="div4" v-for="item in songs" :key="item">
                        ...
                        <div class="div6">
                            <div>
                                ...
                                <div class="div8">
                                    <div style="float:right">
                                        <span class="icon-active" icon="el-icon-link" style="margin-left: 10px;"><i class="el-icon-setting" @click="getEdit(item)"></i></span>
                                        <span class="icon-active" icon="el-icon-link" style="margin-left: 10px;"><i class="el-icon-delete" @click="deleteSong(item.songId)"></i></span>
                                    </div>
                                </div>
                            </div>
                            <div class="div9">
                            ...
                            <div class="div9">
                                谱面定数：{{item.chartConstant}}
                            </div>
                            <div class="div9">
                                加载中文字：{{item.loadingText}}
                            </div>
                            <div class="div9">
                                加载完成文字：{{item.loadedText}}
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        async getCharts(){
            ...
            const{data: res} = await this.$http.post(
                '/user/getPublicCharts',this.params
            );
            if(res.code == 0){
                console.log(res)
                this.songs = [];
        ...
        async delete(songId){
            ...
            const{data: res} = await this.$http.post(
                '/user/deleteChart',this.deleteForm
            );
            if(res.code === 0){
                this.getCharts();
                this.$message({
        ...
        async editStatus(){
            ...
            if(this.value){
                if(this.selectedSong.status == 1){
                    ...
                    const{data: res} = await this.$http.post(
                        '/admin/disaccreditChart',this.song
                    );
                    if(res.code == 0){
                        this.$message.success("调整成功")
                    }
